# Gilbarco ITG Weekly – MySQL version
#
# http://localhost:8000/gilbarco_generate_itg_weekly/65d83ff360d8fb8e5b10b00d
...
gilbarco_itg_weekly_bp = Blueprint("gilbarco_itg_weekly_bp", __name__)

LOG_FUNCTION_CHECK = "Gilbarco_Data_Checking"
LOG_FUNCTION_SELF = "Gilbarco_ITG_Weekly"
CORPID_DEFAULT = "65361d1bc436047c00231e45"


@gilbarco_itg_weekly_bp.route("/gilbarco_generate_itg_weekly/<store_id>", methods=["GET"])
def gilbarco_generate_itg_weekly(store_id):
    log_row_id = None
    ...
    try:
        ...
        if not latest_check or latest_check.get("status") != "Completed":
            ...
            cur.execute(
                ...
                (
                    ...
                    CORPID_DEFAULT,
                    LOG_FUNCTION_SELF,
                    "Latest 'Gilbarco_Data_Checking' was not completed. Aborting itg weekly generation.",
                    "Failed",
                    now,
                ),
            )
            ...
            return jsonify(
                {
                    "message": "Latest 'Gilbarco_Data_Checking' was not completed. Aborting itg weekly generation."
                }
            ), 400
        ...
        # Calculate last complete week's start and end (Monday to Sunday)
        ...
        cur.execute(
            ...
            ("Processing", "Fetching sales data from scan_merge", log_row_id),
        )
        conn.commit()

        # Fetch from scan_merge
        cur.execute(
            """SELECT storeid, corpid, ReportSequenceNumbers, SaleEvents
               FROM scan_merge
        ...
        if not all_sale_events:
            cur.execute(
                """UPDATE scan_connector_logs SET status = %s, message = %s WHERE id = %s""",
                ("Failed", "No data found for the last complete week", log_row_id),
            )
            conn.commit()
            conn.close()
            return jsonify({"message": "No data found for the last complete week"}), 404

        sale_event_json = json.dumps(all_sale_events)
        report_seq_json = json.dumps(report_sequence_numbers)

        cur.execute(
            """INSERT INTO scan_itg_weekly (storeid, corpid, BeginDate, EndDate, ReportSequenceNumbers, SaleEvent)
               VALUES (%s, %s, %s, %s, %s, %s)""",
        ...
        cur.execute(
            """UPDATE scan_connector_logs SET status = %s, message = %s WHERE id = %s""",
            ("Completed", "Weekly sales data inserted successfully", log_row_id),
        )
        conn.commit()
        ...
        return (
            jsonify(
                {
                    "message": "Gilbarco ITG Weekly Sales inserted successfully",
                    "storeid": store_id,
            ...
            if log_row_id:
                ...
                cur.execute(
                    """INSERT INTO scan_connector_logs (log_id, storeid, corpid, function_name, message, status, log_entry_at)
                       VALUES (%s, %s, %s, %s, %s, %s, %s)""",
                    (
                        f"{store_id}_{CORPID_DEFAULT}_log",
                        store_id,
                        CORPID_DEFAULT,
                        LOG_FUNCTION_SELF,
                        str(e),
