"""
Shiftreps v1: single run for hourly schedule. No loop.
Read XML from configured directory; read scan_data_dept, scan_gbtax, scan_gb_department from MySQL;
write scan_msm, scan_mcm, scan_ism, scan_tlm, scan_tpm, scan_cpjr to MySQL.
Logs to scan_connector_logs and scan_connector_status. Run once and exit (e.g. via Task Scheduler every hour).
Uses scan_connector_last_run to process only files newer than last run (Option B: since last run).
"""
import os
import glob
import json
...
# Set via env vars:
#   SCAN_CONNECTOR_LOGS=yes|no   - Write to scan_connector_logs table (default: yes)
#   SCAN_CONNECTOR_STATUS=yes|no - Write to scan_connector_status table (default: yes)
#   DEBUG_PRINT=yes|no          - Print debug lines to console (default: no)
...
SCAN_CONNECTOR_STATUS = _is_enabled("SCAN_CONNECTOR_STATUS", "no")
DEBUG_PRINT = _is_enabled("DEBUG_PRINT", "yes")
LOG_SAVING = _is_enabled("LOG_SAVING", "no")

# Log file for debug output (when DEBUG_PRINT and LOG_SAVING are on). Set SHIFTREPS_DEBUG_LOG for custom path.
...
# --- Option B: "Since last run" (scan_connector_last_run). Cutoff set at main() start. ---
_since_run_cutoff_utc_seconds = None  # Files with mtime > this are processed.

# Fallback when no previous run: process files from last N days (SHIFTREPS_PROCESS_LAST_DAYS, default 1).
PROCESS_LAST_N_DAYS = int(os.environ.get("SHIFTREPS_PROCESS_LAST_DAYS", "1"))


def get_last_run_utc_seconds():
    """Return last run time in UTC seconds since epoch from scan_connector_last_run, or None if no row or zero."""
    ...
    try:
        cur = conn.cursor()
        cur.execute(
            """SELECT last_run_utc_seconds FROM scan_connector_last_run WHERE storeid = %s AND Connector = %s""",
        ...
        debug_print(f"get_last_run_utc_seconds: {e}")
        return None
    finally:
        conn.close()


def set_last_run_utc_seconds(seconds):
    """Write last run time to scan_connector_last_run for this store/connector."""
    conn = get_mysql_connection()
    if not conn:
        return
    try:
        cur = conn.cursor()
        cur.execute(
            """INSERT INTO scan_connector_last_run (storeid, Connector, last_run_utc_seconds)
               VALUES (%s, %s, %s)
               ON DUPLICATE KEY UPDATE last_run_utc_seconds = VALUES(last_run_utc_seconds)""",
            (storeid, CONNECTOR_NAME, seconds),
        )
        conn.commit()
        cur.close()
        debug_print(f"set_last_run_utc_seconds: wrote {seconds}")
    except Exception as e:
        debug_print(f"set_last_run_utc_seconds: {e}")
    finally:
        conn.close()
...
def _get_files_to_process(files, sort_newest_first=False):
    """
    Return list of files to process (Option B: since last run).
    - When _since_run_cutoff_utc_seconds is set: all files with mtime > cutoff, sorted oldest-first.
    - When not set (first run): all files in last PROCESS_LAST_N_DAYS days, oldest-first.
    """
    if not files:
        return []
    if _since_run_cutoff_utc_seconds is not None:
        cutoff = _since_run_cutoff_utc_seconds
    else:
        fallback_days = max(1, PROCESS_LAST_N_DAYS) if PROCESS_LAST_N_DAYS else 1
        cutoff = (datetime.now() - timedelta(days=fallback_days)).timestamp()
    recent = [f for f in files if os.path.getmtime(f) > cutoff]
    recent.sort(key=os.path.getmtime)
    if sort_newest_first:
        return list(reversed(recent))
    return recent  # oldest first (chronological order for inserts)


debug_print(f"Config: directory={directory}, storeid={storeid}, corpid={corpid}, SCAN_CONNECTOR_LOGS={SCAN_CONNECTOR_LOGS}, SCAN_CONNECTOR_STATUS={SCAN_CONNECTOR_STATUS}, DEBUG_PRINT={DEBUG_PRINT}, LOG_SAVING={LOG_SAVING}, PROCESS_LAST_N_DAYS={PROCESS_LAST_N_DAYS}")
if DEBUG_PRINT:
...
# --- XML extractors ---
def _elem_text(elem, default=""):
    """Return element text if element exists, else default. Avoids 'NoneType' has no attribute 'text'."""
    if elem is None:
        return default
    return elem.text if elem.text is not None else default


def extract_msm_data(root):
    """Extract MSM data; use safe defaults for missing elements so malformed XML does not crash."""
    data = {
        "StoreLocationID": _elem_text(root.find(".//TransmissionHeader/StoreLocationID")),
        "BeginDate": _elem_text(root.find(".//MovementHeader/BeginDate")),
        "BeginTime": _elem_text(root.find(".//MovementHeader/BeginTime")),
        "EndDate": _elem_text(root.find(".//MovementHeader/EndDate")),
        "EndTime": _elem_text(root.find(".//MovementHeader/EndTime")),
        "MSMDetail": [],
    }
    for detail in root.findall(".//MSMDetail"):
        misc_code = detail.find(".//MiscellaneousSummaryCodes/MiscellaneousSummaryCode")
        misc_sub = detail.find(".//MiscellaneousSummaryCodes/MiscellaneousSummarySubCode")
        tender = detail.find(".//MSMSalesTotals/Tender")
        tender_code = tender.find("TenderCode") if tender is not None else None
        tender_sub = tender.find("TenderSubCode") if tender is not None else None
        data["MSMDetail"].append({
            "MiscellaneousSummaryCodes": {
                "MiscellaneousSummaryCode": _elem_text(misc_code),
                "MiscellaneousSummarySubCode": _elem_text(misc_sub),
            },
            "MSMSalesTotals": {
                "Tender": {
                    "TenderCode": _elem_text(tender_code),
                    "TenderSubCode": _elem_text(tender_sub),
                },
                "MiscellaneousSummaryAmount": _elem_text(detail.find(".//MSMSalesTotals/MiscellaneousSummaryAmount")),
                "MiscellaneousSummaryCount": _elem_text(detail.find(".//MSMSalesTotals/MiscellaneousSummaryCount")),
            },
        })
    return data
...
# Single run (for hourly scheduling): run all six process steps once, then exit.
# Option B: set "since last run" cutoff so we process only files newer than last run.
debug_print("Main: single run (all jobs)...")
try:
    last_run = get_last_run_utc_seconds()
    if last_run is not None and last_run > 0:
        _since_run_cutoff_utc_seconds = last_run
        debug_print(f"Main: since-run cutoff = {_since_run_cutoff_utc_seconds} (UTC seconds, from DB)")
    else:
        fallback_days = max(1, PROCESS_LAST_N_DAYS) if PROCESS_LAST_N_DAYS else 1
        _since_run_cutoff_utc_seconds = int(time.time()) - 86400 * fallback_days
        debug_print(f"Main: no previous run; cutoff = last {fallback_days} day(s) (UTC seconds {_since_run_cutoff_utc_seconds})")
    ...
    process_tpm_files()
    process_cpjr_files()
    set_last_run_utc_seconds(int(time.time()))
    log_status("Inactive")
